...
#     parser.add_argument("--file_path", type=str, default="./food.pdf")
...
#         pdf_images = extract_images_from_pdf(args.file_path)
#         ocr_results = {}
#         for image_index, pdf_image in enumerate(pdf_images):
#             # Perform OCR on the image
#             receipt_texts, receipt_boxes, receipt_scores, receipt_results = paddle_scan(paddleocr, pdf_image)
#             item_mapping = {}
#             for result in receipt_results:
#                 item_mapping[str(result[0])] = (str(result[1][0]), str(result[1][1]))
#             ocr_results[f"Page_{image_index + 1}"] = item_mapping

#         # Write OCR results to JSON file
#         with open(args.output, "w") as outfile:
#             json.dump(ocr_results, outfile)
#         print("OCR completed and results saved to", args.output)
#     else:
#         # Perform OCR on the image
...
#         # Write OCR results to JSON file
#         with open(args.output, "w") as outfile:
#             json.dump(item_mapping, outfile)
#         print("OCR completed and results saved to", args.output)
...
# # OCR function
# def paddle_scan(paddleocr, img_path_or_nparray):
#     if isinstance(img_path_or_nparray, Image.Image):
#         img_path_or_nparray = np.array(img_path_or_nparray)
#     result = paddleocr.ocr(img_path_or_nparray, cls=True)
#     result = result[0]
#     boxes = [line[0] for line in result]       # Bounding box
#     txts = [line[1][0] for line in result]     # Raw text
